...
#[pyfunction]
fn simulate_paper(py: Python<'_>, payload_json: &str) -> PyResult<String> {
    let result = py.allow_threads(|| run_simulation(payload_json));
    result.map_err(PyValueError::new_err)
}

#[pyfunction]
fn bench_orderbook(py: Python<'_>, payload_json: &str) -> PyResult<String> {
    let result = py.allow_threads(|| run_benchmark(payload_json));
    result.map_err(PyValueError::new_err)
}

#[pymodule]
fn polaris_rs(_py: Python<'_>, module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add_function(wrap_pyfunction!(simulate_paper, module)?)?;
    module.add_function(wrap_pyfunction!(bench_orderbook, module)?)?;
    Ok(())
}

fn run_simulation(payload_json: &str) -> Result<String, String> {
    let payload: InputPayload = serde_json::from_str(payload_json)
        .map_err(|err| format!("invalid simulation payload: {err}"))?;
    let output = simulate(payload);
    serde_json::to_string(&output).map_err(|err| format!("encode simulation output failed: {err}"))
}

fn run_benchmark(payload_json: &str) -> Result<String, String> {
    let total_started = Instant::now();
    let decode_started = Instant::now();
    let payload: BenchPayload = serde_json::from_str(payload_json)
        .map_err(|err| format!("invalid benchmark payload: {err}"))?;
    let decode_ms = decode_started.elapsed().as_secs_f64() * 1000.0;
    let output = bench(payload, decode_ms, total_started);
    serde_json::to_string(&output).map_err(|err| format!("encode benchmark output failed: {err}"))
}
...
fn bench(payload: BenchPayload, decode_ms: f64, total_started: Instant) -> BenchOutput {
    ...
    let total_ms = total_started.elapsed().as_secs_f64() * 1000.0;
    let max_update = update_latency_us.iter().copied().fold(0.0_f64, f64::max);
