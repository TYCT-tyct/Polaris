...
class PolarisSettings(BaseSettings):
    ...
    m4_semantic_llm_enabled: bool = Field(default=True)
    m4_semantic_provider: str = Field(default="minimax")
    m4_semantic_model: str = Field(default="MiniMax-M2.5")
    m4_semantic_api_key: str | None = Field(default=None)
    m4_semantic_base_url: str = Field(default="https://api.minimaxi.com/v1")
    m4_semantic_timeout_sec: float = Field(default=3.0)
    m4_semantic_max_calls: int = Field(default=1)
    m4_semantic_confidence_floor: float = Field(default=0.55)
    m4_semantic_mean_shift_cap: float = Field(default=0.08)
    m4_semantic_uncertainty_cap: float = Field(default=0.20)
    m4_semantic_fail_mode: str = Field(default="neutral_continue")
    m4_semantic_scope: str = Field(default="internal_posts_only")
    m4_semantic_json_strict: bool = Field(default=True)
